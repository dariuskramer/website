describe('Using the generator', () => {
    ...
    it('Simple access requests to companies in "add-all" company pack', () => {
        ...
        cy.get('#send-request-modal-subject')
            ...
            .should(
                'satisfy',
                (subject) =>
                    // Depending on which company loads first, we may get one with `request-language == de` or one
                    // without a `request-language`. And we don't have an `add-all` company pack for an English-speaking
                    // country. :(
        ...
        cy.get('#send-request-modal-body')
            .should('contain.value', 'I am hereby requesting access')
            .should('contain.value', 'Kim Mustermensch');
    ...
    it('Direct marketing objection as PDF, after initially selecting erasure', () => {
        ...
        cy.contains('Microsoft Ireland Operations').click();
        ...
        cy.get('#send-request-modal-body')
            ...
            .should('contain.value', 'Microsoft Ireland Operations')
            .should('not.contain.value', 'Please delete the following personal data')
